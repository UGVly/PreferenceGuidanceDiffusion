...
def get_train_dataloader(args,  accelerator):
    ...
    preprocess_train_fn = get_dataset_preprocessor(args)
...
class HPDv3Dataset(Dataset):
    def __init__(self, split = "train", need_chosen = True, root_path = ""): # /home/jiangzhou/.cache/modelscope/hub/datasets/MizzenAI/HPDv3/
        if split == "train":
            json_path = root_path + "/train.json"
            with open(json_path, "r") as f:
                data = json.load(f)
            model_chosen_list =  [ "flux", "kolors", "sd3", "sdxl", "hunyuan", "real_images", "infinity", "midjourney"]
            if need_chosen:
                data = [item for item in data if item["model1"] in model_chosen_list and item["model2"] in model_chosen_list]
        else:
            json_path = root_path +"/test.json"
            with open(json_path, "r") as f:
                data = json.load(f)
        data = [d for d in data if os.path.exists(os.path.join(root_path, d["path1"])) and os.path.exists(os.path.join(root_path, d["path2"]))]
        self.data = data
        self.root_path = root_path

        self.train_pad_to_square = transforms.Lambda(
        ...
        self.train_resize = transforms.Resize((512, 512), interpolation=transforms.InterpolationMode.BILINEAR)
        # train_crop = transforms.RandomCrop(512) if args.random_crop else transforms.CenterCrop(512)
    ...
    def transform(self, img_path):
        ...
        if random.random() < 0.5:
            img = self.train_pad_to_square(img)
    ...
    def __getitem__(self, idx):
        ...
        img_path1 = os.path.join(self.root_path, path1)
        img_path2 = os.path.join(self.root_path, path2)

        img1 = self.transform(img_path1)
        img2 = self.transform(img_path2)

        pixel_values = torch.cat([img1, img2], dim=0)
        return {"pixel_values": pixel_values, "prompt": prompt, "caption": prompt, "chosen": img1, "rejected": img2}
...
def get_dataset_preprocessor(args, tokenizer_one = None, tokenizer_two = None):
    # Preprocessing the datasets.
